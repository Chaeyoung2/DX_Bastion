...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	m_MainSplitter.CreateView(0, 1, RUNTIME_CLASS(CTOOLView),
		CSize(WINCX, WINCY), pContext);


	m_SecondSplitter.CreateStatic(&m_MainSplitter, 2, 1);

	m_SecondSplitter.CreateView(0, 0, RUNTIME_CLASS(CMiniView),
								CSize(400, int(WINCY * 0.3f)),
								pContext);

	m_SecondSplitter.CreateView(1, 0, RUNTIME_CLASS(CMyForm),
								CSize(400, int(WINCY * 0.7f)),
								pContext);

	m_MainSplitter.SetColumnInfo(0, 400, int(WINCY * 0.3f));
